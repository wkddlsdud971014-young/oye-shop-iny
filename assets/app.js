...
function paintCheckout() {
  ...
  // 결제할 상품 목록을 장바구니에서 읽어 온다 - 상품 목록에 없는 상품은 뺀다
  const cartItems = Cart.read().filter(i => findProduct(i.id));
  // 장바구니 합계 - 상품 가격 × 수량의 합이고 배송비는 넣지 않는다
  ...
  dataLayer.push({
    ...
    // 배송비가 없는 주문인가 - 합계 50000 이상이면 yes, 아니면 no (ecommerce 밖에 둔다)
